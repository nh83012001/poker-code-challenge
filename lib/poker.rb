class Poker
  ...
  # empty objects and itterators for creating
  @player1Hands = {}
  @player2Hands = {}
  @x = 1
  @counter = 0
  ...
  # Start of section for object keys on hand
  # -------------------------------------------------------------------
  def self.flush?(hand)
    hand.uniq.length == 1
  end

    def self.uniqueValues(hand)
      hand.uniq.length
    end
    ...
    def self.straight?(hand)
      ...
      # checking for hi and lo ace
       ...
       straight =  hand &([minVal,minVal+1,minVal+2,minVal+3,minVal+4])
    ...
    def self.pair?(hand)
      if hand.uniq.length == 5
      return false
      else
      return hand.inject(Hash.new(0)) { |h,v| h[v] += 1; h }
      end
    end
    ...
    # send back descending array of 5 cards values
    def self.descendingValue(hand)
      hand.sort! {|x,y| y <=> x }
    end
    ...
    def self.rank(handObject)
      ...
      elsif (handObject[:uniqueValues]==2 && handObject[:largestPair][1] == 3)
      return 7
      #  Flush: All cards of the same suit.
      elsif (handObject[:flush] == true)
      return 6
      # Straight: All cards are consecutive values.
      elsif (handObject[:straight] == true)
      return 5
      ...
      # --------------------------------------------------------------------
      # End of object keys section

# Divvy up of the hands, creating of key/values for each hand, ends with rank of each hand
    @allHands[0].each do |i|
    handSuits = []
    handValues = []
      i.each do |x|
                case x[0].chr
                when 'T' then handValues.push(10)
                when 'J' then handValues.push(11)
                when 'Q' then handValues.push(12)
                when 'K' then handValues.push(13)
                when 'A' then handValues.push(14)
                else  handValues.push(x[0].chr.to_i)
                end
                handSuits.push(x[1].chr)
      end
        if @counter.even?
            @player1HandCounter = "hand" + @player1Counter.to_s
            @player1Hands[@player1HandCounter] = {:hand => i}
            @player1Hands[@player1HandCounter][:descendingValue] = descendingValue(handValues)
            @player1Hands[@player1HandCounter][:flush] = flush?(handSuits)
            @player1Hands[@player1HandCounter][:straight] = straight?(handValues)
            @player1Hands[@player1HandCounter][:pair] = pair?(handValues)
            @player1Hands[@player1HandCounter][:largestPair] = largestPair(@player1Hands[@player1HandCounter][:pair])
            @player1Hands[@player1HandCounter][:uniqueValues] = uniqueValues(handValues)
            @player1Hands[@player1HandCounter][:rank] = rank(@player1Hands[@player1HandCounter])
            @player1Counter = @player1Counter + 1
        end
        if !@counter.even?
            @player2HandCounter = "hand" + @player2Counter.to_s
            @player2Hands[@player2HandCounter] = {:hand => i}
            @player2Hands[@player2HandCounter][:descendingValue] = descendingValue(handValues)
            @player2Hands[@player2HandCounter][:flush] = flush?(handSuits)
            @player2Hands[@player2HandCounter][:straight] = straight?(handValues)
            @player2Hands[@player2HandCounter][:pair] = pair?(handValues)
            @player2Hands[@player2HandCounter][:largestPair] = largestPair(@player2Hands[@player2HandCounter][:pair])
            @player2Hands[@player2HandCounter][:uniqueValues] = uniqueValues(handValues)
            @player2Hands[@player2HandCounter][:rank] = rank(@player2Hands[@player2HandCounter])
  ...
  def self.highCardTiebreak (hand)
    ...
    # This also never happens, but is itterating over values and checking each card in order
    elsif @player2Hands[hand][:descendingValue][0] == @player1Hands[hand][:descendingValue][0]
        a=0
        while @player1Hands[hand][:descendingValue][a] == @player2Hands[hand][:descendingValue][a] && a<4
          if @player1Hands[hand][:descendingValue][a+1] > @player2Hands[hand][:descendingValue][a+1]
            @player1Wins = @player1Wins +1
            elsif @player2Hands[hand][:descendingValue][a+1] > @player1Hands[hand][:descendingValue][a+1]
            @player2Wins = @player2Wins +1
          end
        end
      else
  ...
  def self.highPairTiebreak (hand)
   if @player1Hands[hand][:largestPair][0] > @player2Hands[hand][:largestPair][0]
      ...
      @player2Wins = @player2Wins +1
    #tied pair, go to high card  (happens once)
  ...
  while @i <= 1000
    ...
    if @player1Hands[hand][:rank] > @player2Hands[hand][:rank]
      @player1Wins = @player1Wins +1
    elsif @player2Hands[hand][:rank] > @player1Hands[hand][:rank]
      @player2Wins = @player2Wins +1
      # tiebreaker for high card, straight, flush, and straight flush
    elsif @player1Hands[hand][:rank] == @player2Hands[hand][:rank] &&  [1,5,6,9].include?(@player2Hands[hand][:rank])
      highCardTiebreak(hand)
      # tiebreaker for pair, 2 pair, 3 of a kind, full house, and four of a kind
